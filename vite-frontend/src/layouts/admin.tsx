...
export default function AdminLayout({
  ...
  return (
    <div
      className={`flex ${isMobile ? "min-h-screen" : "h-screen"} bg-background`}
    >
      {/* 移动端遮罩层 */}
      ...
      {/* 主内容区域 */}
      <div
        className={`flex flex-col flex-1 min-w-0 ${isMobile ? "min-h-0" : "h-full overflow-hidden"}`}
      >
        {/* 顶部导航栏 */}
        ...
        {/* 主内容 */}
        <main
          className={`flex-1 bg-background ${isMobile ? "" : "overflow-y-auto"}`}
        >
          {children}
        </main>
      </div>
