...
export const upgradeNode = (data: {
  id: number;
  version?: string;
  url?: string;
  sha256?: string;
  arch?: string;
}) => Network.post("/node/upgrade", data);
...
// 批量操作
export const batchDeleteForward = (ids: number[]) =>
  Network.post("/forward/batch-delete", { ids });
export const batchPauseForward = (ids: number[]) =>
  Network.post("/forward/batch-pause", { ids });
export const batchResumeForward = (ids: number[]) =>
  Network.post("/forward/batch-resume", { ids });
export const batchChangeTunnelForward = (ids: number[], tunnelId: number) =>
  Network.post("/forward/batch-change-tunnel", { ids, tunnelId });
